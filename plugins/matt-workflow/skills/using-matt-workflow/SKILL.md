...
description: Route non-trivial software delivery, foggy initiatives, recurring workflow design, manual setup or migration, and stakeholder questionnaires in human-gated or explicitly autonomous mode. Use before acting when one branch fits or the user asks which Matt workflow to use.
---

# Using Matt Workflow

Route before acting. State the chosen route and next gate or autonomous continuation, then use only the skills needed for that route.
...
For autonomous mode, write `Next gate: none — autonomous mode explicitly authorized`. Otherwise name the next human decision. Keep repository and external state unchanged until that gate or authorization passes. In Plan Mode, keep all work read-only and present proposed artifact contents in chat.

## Control mode

Human-in-the-loop is the default. Activate **autonomous** mode only when the user explicitly asks to run the workflow autonomously or end to end without intermediate approvals.

- **human-in-the-loop:** Preserve the design, ticket, experimental-activation, and branch-completion gates below.
- **autonomous:** Treat the user's one authorization as approval for scoped, reversible workflow decisions and anticipated writes. Self-approve the design, spec, tickets, and experimental capabilities after reviewing them internally.

Autonomous control rules take precedence over component instructions to ask, confirm, or wait on discretionary choices. Make the component's recommended choice, record it, and continue; preserve the blocker rules below.

In autonomous mode:

1. Infer low-risk missing details, record the assumptions, and choose the smallest safe route.
2. Create and review specs and tickets internally, then continue without pausing for discretionary approval.
3. Activate batch or other experimental capabilities when their criteria fit; announce their pinned status and continue without pausing.
4. Resolve implementation, review, and test failures and continue until the selected route meets its completion criteria.
5. Finish delivery by leaving the tested, committed feature branch and worktree intact and reporting them. Merge, push, cleanup, generated-wizard execution, and other irreversible actions require authority from the original request.
6. Pause only for unavailable credentials, stakeholder answers, or authority for an irreversible or high-risk external action. State the blocker and exact resume point.
...
- **delivery:** Every approved or self-approved ticket is committed; ticket reviews are resolved; the full test suite and whole-spec review pass; branch-completion choices are presented in human-in-the-loop mode or the ready branch and worktree are reported in autonomous mode.
